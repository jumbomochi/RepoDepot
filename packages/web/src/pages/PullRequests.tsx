...
function PullRequests() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchPullRequests, 60000);
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div>
          ...
          <p style={{ fontSize: '1rem', color: '#9ca3af', marginTop: '0.5rem' }}>
            {pullRequests.length} pull requests tracked
          </p>
        </div>
      ...
      {pullRequests.length === 0 ? (
        ...
        >
          ...
          <p style={{ fontSize: '1rem', color: '#9ca3af' }}>
            Pull requests will appear here once repositories are synced
          </p>
        </div>
      ) : (
        <PRKanbanBoard pullRequests={pullRequests} />
      )}
    </div>
  );
}
